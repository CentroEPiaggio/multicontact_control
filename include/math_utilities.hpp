...
#ifndef __MATH_UTILITIES_HPP
#define __MATH_UTILITIES_HPP
...
static Eigen::Matrix<double,31,18> pseudoInverseQR_3118(const Eigen::Matrix<double,18,31> &a, double epsilon = std::numeric_limits<double>::epsilon())
{
  Eigen::HouseholderQR<Eigen::Matrix<double,31,18>> qr(a.transpose());

  Eigen::Matrix<double,31,18> Rtot = qr.matrixQR().template triangularView<Eigen::Upper>();
...
static Eigen::Matrix<double,37,27> pseudoInverseQR_3727(const Eigen::Matrix<double,27,37> &a, double epsilon = std::numeric_limits<double>::epsilon())
{
  Eigen::HouseholderQR<Eigen::Matrix<double,37,27>> qr(a.transpose());

  Eigen::Matrix<double,37,27> Rtot = qr.matrixQR().template triangularView<Eigen::Upper>();
